...
These are filters that can be applied in the [`statind`](#statind) stage via the `'Preprocess'` keyword argument:
...
files = filelistall(dir_0("GEMS-MagTIP-insider/src/tools"))
y = [getdoc(f, filename_header=3) for f in files]
