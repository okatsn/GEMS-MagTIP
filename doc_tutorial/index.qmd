...
::: {.content-visible when-profile="zhtw"}
[GEMS-MagTIP 的主要功能](/doc_library/index.qmd#main-functions) 需要以目錄中的中間數據（如 `.mat` 文件）作為輸入參數，並將輸出數據存放於指定的目錄中。
...
`station_location.csv` 指定了每個測站的位置；
以下為 `station_location.csv` 的部分內容：
:::
...
有關更多信息，請參閱[加載原始數據並將其轉換為標準格式](/doc_library/#load-raw-data-and-convert-it-into-a-standard-format)部分。
:::
...
::: {.content-visible when-profile="zhtw"}
::: {.callout-tip}
- 您可以使用 [`mkdir_default`](/doc_library/#mkdir_default) 自動生成主要功能所需的空目錄。
